...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    h = [h0]
    for t in range(x.shape[1]): # number of input
        next_h, cache_t = rnn_step_forward(x[:, t], h[t], Wx, Wh, b) # input t
        h.append(next_h)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        dx_t, dh0, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:, t] + dh0, cache[t]) # add the accumulated gradient from future time steps
        dx[:, t] = dx_t
        # Chain rule: dL/dWx = sum (dL/dh_t*dh_t/dW_x) => add gradient
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def word_embedding_backward(dout, cache):
    ...
    x, W = cache
    dW = np.zeros_like(W)
    np.add.at(dW, x, dout) # update dW at position x by dout
    pass
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    a = np.hsplit(x @ Wx + prev_h @ Wh + b, 4) # split to 4 equal part (Wx, Wh, b have 4 part)
    f = sigmoid(a[0]) # forget
    i = sigmoid(a[1]) # input
    o = sigmoid(a[2]) # output
    g = np.tanh(a[3]) # gate
    next_c = f*prev_c + i*g
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # add to total gradient by chain rule
    dnext_c += dnext_h * o * (1 - np.square(np.tanh(next_c))) # dtanh/dx = 1 - tanh(x)^2
    # next_c = f*prev_c + i*g
    dprev_c = dnext_c * f
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x.shape[1]): # number of input
        h, c, cache_t = lstm_step_forward(x[:, t], hs[-1], c, Wx, Wh, b) # input t
        hs.append(h)
        cache.append(cache_t)
    # Stack over T, excluding h0
    hs = np.stack(hs[1:], axis=1)
    pass
