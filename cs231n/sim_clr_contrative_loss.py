...
# Distinguishes between "positive pairs" (different augmented views of the same image) and "negative pairs" (views of different images) to have features.
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # positive pair (two differently augmented images of the same original image) = -log(exp(sim(zk,zkN))/sum{exp(sim(zk,zm))})
        # negative pair: l(k+N) = -log(exp(sim(zkN,zk))/sum{exp(sim(zkN,zm))})
